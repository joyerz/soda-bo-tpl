...
type Props = {
  ...
  // data: Array<{
  //   title: string,
  //   items: Array<string | React.Node | () => React.Node>
  // }>
}
...
const index = (props: Props) => {
  ...
  return (
    <div styleName="result-detail">
      ...
      {convertData(data, '设备状态', status).map((item, idx) => (
        <InfoBox key={item.title + '_' + idx} {...item} />
      ))}

      {convertData(data, '功能检测', ability).map((item, idx) => (
        <InfoBox key={item.title + '_' + idx} {...item} />
      ))}

      {convertData(data, '设备数据', [], [].concat(ability, status)).map((item, idx) => (
        <InfoBox key={item.title + '_' + idx} {...item} />
      ))}
  ...
  if (typeof data === 'object') {
    Object.keys(data).forEach((key) => {
      if(
        ...
        let value = data[key]
        dataMap(key, value) && result.push(dataMap(key, value, true, data))
      }
    })
  }
